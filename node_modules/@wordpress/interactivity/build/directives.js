  ...
  if (!contextObjectToProxy.has(current)) {
    const proxy = new Proxy(current, {
      get: (target, k) => {
        ...
        if (k in target && !contextAssignedObjects.get(target)?.has(k) && (0, _utils.isPlainObject)(currentProp)) {
...
const updateContext = (target, source) => {
  for (const k in source) {
    if ((0, _utils.isPlainObject)((0, _proxies.peek)(target, k)) && (0, _utils.isPlainObject)(source[k])) {
      updateContext((0, _proxies.peek)(target, k), source[k]);
    } else if (!(k in target)) {
      target[k] = source[k];
    }
  }
...
var _default = () => {
  ...
  (0, _hooks2.directive)('each', ({
    ...
    return list.map(item => {
      ...
      const itemContext = proxifyContext((0, _proxies.proxifyState)(namespace, {}), inheritedValue[namespace]);
      const mergedContext = {
        ...inheritedValue,
